    ...
    <main class="admin-main">
      <div class="admin-content">
        ...
        <div class="profile-sections">
          ...
          <div class="profile-section">
            <div class="profile-section-header">
              <h2 class="profile-section-title">Floor Management</h2>
              <p class="profile-section-desc">Control how long manual occupied overrides stay active on today's floor.</p>
            </div>
            <div class="profile-section-body">
              <form method="post" action="../../actions.php?action=save_settings" id="floorSettingsForm">
                <input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>">
                <input type="hidden" name="action_token" value="<?= e(action_token('save_settings')) ?>">
                <input type="hidden" name="section" value="floor">
                <div class="form-group" style="margin-bottom: var(--space-5); max-width: 16rem;">
                  <label for="floorManualOccupiedMinutes" class="form-label">Manual Occupied Duration</label>
                  <div style="display:flex; align-items:center; gap:var(--space-3);">
                    <input
                      type="number"
                      id="floorManualOccupiedMinutes"
                      name="floor_manual_occupied_minutes"
                      class="form-input"
                      min="5"
                      max="720"
                      step="5"
                      value="<?= e($settings['floor_manual_occupied_minutes'] ?? '120') ?>"
                      style="max-width: 8rem;">
                    <span class="text-muted text-sm">minutes</span>
                  </div>
                </div>
                <div style="display:flex; justify-content:flex-end;">
                  <button type="submit" class="btn btn-primary">Save Floor Rules</button>
                </div>
              </form>
            </div>
          </div>
